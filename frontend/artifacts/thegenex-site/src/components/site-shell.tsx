...
export function Header() {
  ...
  return <header className="fixed inset-x-0 top-0 z-40 border-b border-foreground/[.08] bg-background/80 backdrop-blur-xl">
    <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-5 lg:px-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="grid h-10 w-10 place-items-center rounded-full border border-foreground/10 text-muted-foreground md:hidden" onClick={() => setOpen(!open)} aria-label="Toggle menu" data-testid="button-mobile-menu">{open ? <X size={18} /> : <Menu size={18} />}</button>
      </div>
    </div>
    {open && <div className="border-t border-foreground/[.08] bg-background px-5 py-6 md:hidden">
      {nav.map(item => <Link key={item.href} href={item.href} onClick={() => go(item.label)} className="block border-b border-foreground/[.08] py-4 font-display text-2xl" data-testid={`link-mobile-${item.label.toLowerCase()}`}>{item.label}</Link>)}
...
export function Footer() {
  return <footer className="border-t border-foreground/[.1] bg-background">
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-3 border-t border-foreground/[.08] px-5 py-5 font-mono text-[10px] uppercase tracking-[.12em] text-muted-foreground sm:flex-row sm:items-center sm:justify-between lg:px-8"><span>© {new Date().getFullYear()} theGenEx</span><span className="flex gap-4"><Link href="/privacy" data-testid="link-privacy">Privacy</Link><Link href="/terms" data-testid="link-terms">Terms</Link></span></div>
  </footer>;
}
