...
import { ButtonLink, SectionLabel, SiteShell } from '@/components/site-shell';
...
export function ContactPage() {
  ...
  return <SiteShell><main className="mx-auto grid max-w-7xl gap-14 px-5 pb-28 pt-36 lg:grid-cols-[.8fr_1.2fr] lg:px-8">
    ...
    <form onSubmit={submit} onFocus={() => trackEvent('contact_form_open', undefined, '/contact')} noValidate className="rounded-xl border border-foreground/10 bg-card/60 p-6 sm:p-9">
      {sent ? <div className="flex min-h-[420px] flex-col items-start justify-center">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <label className="text-sm text-muted-foreground">Name<input required maxLength={100} name="name" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-name" />{errors.name && <span className="mt-1 block text-xs text-destructive">{errors.name}</span>}</label>
          <label className="text-sm text-muted-foreground">Work email<input required type="email" maxLength={254} name="email" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-email" />{errors.email && <span className="mt-1 block text-xs text-destructive">{errors.email}</span>}</label>
        </div>
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          <label className="text-sm text-muted-foreground">Company <span className="text-muted-foreground/60">(optional)</span><input maxLength={150} name="company" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-company" /></label>
          <label className="text-sm text-muted-foreground">Phone <span className="text-muted-foreground/60">(optional)</span><input maxLength={30} name="phone" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-phone" /></label>
        </div>
        <label className="mt-5 block text-sm text-muted-foreground">What do you want to build?<select required name="requirement" defaultValue="" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="select-requirement"><option value="" disabled>Select one</option>{REQUIREMENT_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select>{errors.requirement && <span className="mt-1 block text-xs text-destructive">{errors.requirement}</span>}</label>
        <label className="mt-5 block text-sm text-muted-foreground">Budget <span className="text-muted-foreground/60">(optional)</span><input maxLength={60} name="budget" placeholder="e.g. $5k–15k" className="mt-2 w-full rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-budget" /></label>
        <label className="mt-5 block text-sm text-muted-foreground">Anything else? <span className="text-muted-foreground/60">(optional)</span><textarea maxLength={5000} name="message" rows={5} className="mt-2 w-full resize-none rounded-md border border-foreground/10 bg-background px-3 py-3 text-foreground outline-none focus:border-primary" data-testid="input-message" />{errors.message && <span className="mt-1 block text-xs text-destructive">{errors.message}</span>}</label>
